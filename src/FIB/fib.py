...
def fibonacci(n, k):
    if(n == 1 or n == 2):
        ...
        r = fibonacci(n-1, k) + k * fibonacci(n-2, k)
        
    return r

def fib_rabbits(path):
    n, k = convert_int(path)
    print(fibonacci(n, k))

path = "src/FIB/rosalind_fib.txt"
fib_rabbits(open_file(path))
